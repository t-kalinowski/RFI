...
some of the SEXP objects. For example, we know the subroutine above only
modifies the first argument, `array`, and does not modify the second argument,
`shift`. We can use that and skip duplicating `shift` and only duplicate `array`
like so.
```{r}
cshift <- function(array, shift) {
  RFI::.ModernFortran(func_ptr, 
